...
export default function ChatBubble({ message, userAvatarUrl, userInitial }: ChatBubbleProps) {
    ...
    return (
      <div className="flex justify-end items-start gap-2.5 mb-6">
        <div className="max-w-[85%] sm:max-w-[70%]">
          ...
          <div className="bg-brand/15 border border-brand/25 text-fg rounded-2xl rounded-tr-sm px-4 py-3">
            <p className="text-sm font-medium leading-relaxed whitespace-pre-wrap">{message.text}</p>
          </div>
        </div>
        <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-brand/25 bg-overlay shrink-0 mt-5 flex items-center justify-center">
          {userAvatarUrl ? (
            <Image
              src={userAvatarUrl}
              alt="You"
  ...
  return (
    <div className="flex items-start gap-3 mb-6">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div
          className={
            "rounded-2xl rounded-tl-sm px-4 py-3 max-w-full sm:max-w-[85%] " +
            (message.error
