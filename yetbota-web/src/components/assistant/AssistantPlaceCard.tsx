...
export default function AssistantPlaceCard({ place }: { place: PlaceCard }) {
  return (
    <div className="mt-3 bg-surface border border-border-subtle rounded-xl overflow-hidden flex gap-3 p-3">
      {/* Thumbnail */}
      ...
      <div className="flex flex-col justify-between flex-1 min-w-0">
        <div>
          <p className="text-fg text-sm font-bold leading-tight truncate">{place.name}</p>
          <div className="flex items-center gap-1.5 mt-1">
            <Star className="w-3 h-3 text-yellow-400 fill-yellow-400 shrink-0" />
            <span className="text-yellow-400 text-xs font-semibold">{place.rating}</span>
            <span className="text-fg-faint text-xs">• {place.reviewCount} reviews</span>
          </div>
          <div className="flex items-center gap-1.5 mt-1">
            <span className={`text-xs font-semibold ${place.isOpen ? "text-brand" : "text-red-400"}`}>
              {place.isOpen ? "Open now" : "Closed"}
            </span>
            <span className="text-fg-faint text-xs">• {place.distance}</span>
          </div>
        </div>
        <a
