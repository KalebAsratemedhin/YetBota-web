...
func NewPostWorkflow(ctx workflow.Context, input processorsDomain.NewPostWorkflowInput) error {
	...
	var embeddingResult processorsDomain.PostEmbeddingResult
	if err := workflow.ExecuteChildWorkflow(embeddingCtx,
		constants.PostEmbeddingWorkflowName,
		processorsDomain.PostEmbeddingWorkflowInput(input)).Get(ctx, &embeddingResult); err != nil {
		logger.Error("PostEmbedding workflow failed, continuing to feed update", "postID", input.PostID, "error", err)
	} else if embeddingResult.Verdict == constants.VerdictDuplicate {
		// Post is a duplicate: mark it hidden and notify the author. Skip the
		// feed update so the duplicate never reaches anyone's feed.
		if err := workflow.ExecuteActivity(ctx, (*newPostActivity).MarkPostDuplicate, input.PostID).Get(ctx, nil); err != nil {
			logger.Error("MarkPostDuplicate failed", "postID", input.PostID, "error", err)
		}
		logger.Info("post marked duplicate, skipping feed update", "postID", input.PostID)
		return nil
	}
